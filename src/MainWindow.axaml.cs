...
namespace LoopLauncher
{
    public partial class MainWindow : Window
    {
            ...
            ModpackService modpackService)
        {
            ...
            LogService.LogGame("=== LoopLauncher starting ===");
        ...
        private void UpdateUI()
        {
            ...
            ToolTip.SetTip(OpenModsButton, _localization.Get("main.manage_modpacks"));
            ...
            ToolTip.SetTip(ReinstallButton, _localization.Get("main.reinstall"));
        ...
        private async Task CheckForUpdatesAsync()
        {
            ...
            if (update != null)
            {
                ...
                var box = MessageBoxManager.GetMessageBoxStandard(
                    ...
                    hasPortable ? ButtonEnum.YesNoCancel : ButtonEnum.YesNo,
                    MsBox.Avalonia.Enums.Icon.Info);

                var result = await box.ShowAsync();

                if (result == ButtonResult.Yes && hasPortable)
                {
                    // Автообновление
                    await PerformAutoUpdateAsync(update);
                }
                else if (result == ButtonResult.No && hasPortable)
                {
        ...
        private async void BranchComboBox_SelectionChanged(object sender, Avalonia.Controls.SelectionChangedEventArgs e)
        {
            if (IsLoaded)
            {
                await LoadVersionsAsync();
        ...
        private void VersionComboBox_SelectionChanged(object sender, Avalonia.Controls.SelectionChangedEventArgs e)
        {
            if (IsLoaded)
            {
                UpdateReinstallButtonVisibility();
        ...
        private async void PlayButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(nickname))
            {
                ...
                MessageBoxManager.GetMessageBoxStandard(
                                _localization.Get("error.nickname_empty"),
                                _localization.Get("error.title"),
                                ButtonEnum.Ok,
            ...
            if (nickname.Length < 3 || nickname.Length > 16)
            {
                ...
                MessageBoxManager.GetMessageBoxStandard(
                                _localization.Get("error.nickname_length"),
                                _localization.Get("error.title"),
                                ButtonEnum.Ok,
                                MsBox.Avalonia.Enums.Icon.Warning).ShowAsync();
                return;
            ...
            if (selectedVersion == null)
            {
                ...
                MessageBoxManager.GetMessageBoxStandard(
                                _localization.Get("error.version_select"),
                                _localization.Get("error.title"),
                                ButtonEnum.Ok,
                                MsBox.Avalonia.Enums.Icon.Warning).ShowAsync();
                return;
        ...
        async private void Mods_Click(object sender, PointerReleasedEventArgs e)
        {
            ...
            if (modsWindow != null)
            {
                await modsWindow.ShowDialog(this);
                // Refresh modpacks after mods window closes (user might have created/deleted modpacks)
                LoadModpacks();
            }
        }

        async private void OpenMods_Click(object sender, RoutedEventArgs e)
        {
            var modsWindow = (Application.Current as App)?.Services.GetRequiredService<ModsWindow>();
        ...
        private void ModpackComboBox_SelectionChanged(object sender, Avalonia.Controls.SelectionChangedEventArgs e)
        {
            if (!IsLoaded) return;
        ...
        private void Website_Click(object sender, PointerReleasedEventArgs e)
        {
            try
            {
                Process.Start(new ProcessStartInfo
                {
                    FileName = "https://LoopLauncher.ru",
                    UseShellExecute = true
                });
            }
            catch { }
        ...
        private void CheckServerAvailable()
        {
            ...
            // before Visibility.Collapsed on "false" case
            StartServerButton.IsVisible = !string.IsNullOrEmpty(serverBatPath) 
                ? true 
                : false;
        }
        ...
        private void StartServerButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBoxManager.GetMessageBoxStandard(
                    string.Format(_localization.Get("error.launch"), ex.Message),
                    _localization.Get("error.title"),
                    ButtonEnum.Ok,
                    MsBox.Avalonia.Enums.Icon.Error).ShowAsync();
            }
        ...
        private void UpdateReinstallButtonVisibility()
        {
            ...
            {
                ReinstallButton.IsVisible = false;
            }
            else
            {
                //before Visibility.Collapsed
