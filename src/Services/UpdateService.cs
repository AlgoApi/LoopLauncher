...
namespace LoopLauncher.Services
{
    ...
    public class UpdateService
    {
        ...
        private const string RepoOwner = "AlgoApi";
        private const string RepoName = "LoopLauncher";
        ...
        public UpdateService()
        {
            ...
            _httpClient.DefaultRequestHeaders.Add("User-Agent", "LoopLauncher");
        ...
        public async Task<bool> DownloadAndApplyUpdateAsync(UpdateInfo update, LocalizationService localization)
        {
            ...
            var tempDir = Path.Combine(Path.GetTempPath(), "LoopLauncher_Update");
            var zipPath = Path.Combine(tempDir, $"LoopLauncher_Portable_{update.Version}.zip");
            ...
            try
            {
                ...
                var exePath = Process.GetCurrentProcess().MainModule?.FileName ?? 
                    Path.Combine(launcherDir, "LoopLauncher.exe");

                // PowerShell скрипт вместо batch - меньше детектов
                var psContent = $@"
# LoopLauncher Update Script
