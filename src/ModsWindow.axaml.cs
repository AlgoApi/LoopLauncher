...
namespace LoopLauncher
{
    public partial class ModsWindow : Window
    {
        ...
        public ModsWindow() { InitializeComponent(); }

        protected override async void OnOpened(EventArgs e)
        {
            base.OnOpened(e);

            // Тяжелые асинхронные задачи выполняем здесь
        ...
        {
            if (sender is Button element && element.Tag is InstalledMod mod)
            {
        ...
        {
            if (sender is Button element && element.Tag is CurseForgeSearchResult mod)
            {
                element.IsEnabled = false;
                ProgressBar.IsVisible = true;
        ...
        private void RefreshTagFilterPanel()
        {
            ...
            // before Visibility.Collapsed
            TagFilterPanel.IsVisible = tags.Count > 0 ? true : false;
        ...
        private void UpdateTagChipSelection()
        {
            ...
            // Используем TryGetResource, чтобы избежать исключения, если ресурс не найден
            this.TryGetResource("AccentBrush", this.ActualThemeVariant, out object? accentResource);
            var accentBrush = accentResource as IBrush ?? Brushes.Blue; // Fallback цвет, если ресурс не найден

            // 2. Парсинг цвета из Hex
            var hexColor = Color.Parse("#3d444d");
            var backgroundBrush = new SolidColorBrush(hexColor);
        ...
        async private void ApplyTagFilter()
        {
            ...
            await Dispatcher.UIThread.InvokeAsync(() =>
            {
                ...
                {
                    var container = InstalledModsList.ContainerFromItem(item);
                    
                    // FrameworkElement заменен на Control
                    UpdateModItemCheckboxVisibility(container as Control);
                }
            }, DispatcherPriority.Background);
        }
        
        private void AllTagsChip_Click(object sender, PointerPressedEventArgs e)
        {
            _selectedTagId = null;
            UpdateTagChipSelection();
            ApplyTagFilter();
        ...
        private void TagChip_Click(object sender, PointerPressedEventArgs e)
        {
            if (sender is Control element && element.Tag is ModTag tag)
            {
        ...
        private void InstalledMod_RightClick(object sender, PointerPressedEventArgs e)
        {
        ...
        {
            if (sender is ContextMenu contextMenu && contextMenu.PlacementTarget is Button element)
            {
                ...
                if (addTagMenuItem != null)
                {
                    ...
                    foreach (var tag in allTags.Where(t => !modTags.Contains(t.Id)))
                    {
                        ...
                        // 1. Вместо Style = (Style)FindResource...
                        // В Avalonia стили задаются через классы:
                        tagItem.Classes.Add("ModernMenuItem"); 

                        // 2. Вместо ColorConverter.ConvertFromString
                        // Используем Color.Parse
                        var tagColor = Color.Parse(tag.Color);

                        // 3. Создаем иконку
                        tagItem.Icon = new Border
                        {
                            Width = 12,
                            Height = 12,
        ...
        {
            var mod = (sender as Button)?.Tag as InstalledMod;
            
            // 1. Настройка окна в Avalonia
            // WindowStyle и AllowsTransparency здесь заменяются на SystemDecorations и TransparencyLevelHint
            var dialog = new Window
            {
                Title = _localization.Get("mods.tags.create_title"),
                Width = 300,
                Height = 220, // Увеличил высоту, так как в Avalonia размеры могут отличаться
            ...
            // 2. Поиск ресурсов в Avalonia (FindResource -> TryGetResource или FindResource у объекта)
            // Используем безопасное приведение к IBrush
            var cardBg = this.FindResource("CardBackgroundBrush") as IBrush;
            var borderBr = this.FindResource("BorderBrush") as IBrush;
            var textBr = this.FindResource("TextBrush") as IBrush;
            ...
            var titleText = new TextBlock
            {
                ...
                FontSize = 14,
                FontWeight = FontWeight.Bold, // FontWeight вместо FontWeights
                Foreground = textBr,
            ...
            // В Avalonia стиль применяется через классы (Classes.Add), 
            // либо создается неявный стиль в XAML. Напрямую свойство Style в коде менять нельзя (read-only).
            nameBox.Classes.Add("ModernTextBox"); 
            ...
            foreach (var color in colors)
            {
                var colorBorder = new Border
                {
                    ...
                    // ColorConverter заменен на Color.Parse
                    Background = new SolidColorBrush(Color.Parse(color)),
                    Margin = new Thickness(0, 0, 6, 0),
                    Cursor = new Cursor(StandardCursorType.Hand), // Cursor вместо Cursors
                ...
                // MouseLeftButtonDown заменен на PointerPressed
                colorBorder.PointerPressed += (s, args) =>
                {
                    if (selectedColorBorder != null)
                        selectedColorBorder.BorderBrush = Brushes.Transparent;
                    
                    var b = (Border)s!;
                    selectedColor = (string)b.Tag!;
                    b.BorderBrush = Brushes.White;
                    selectedColorBorder = b;
                };
            ...
            var cancelBtn = new Button
            {
                ...
                // Замена ColorConverter и Styles
                Background = new SolidColorBrush(Color.Parse("#3d444d")),
                Padding = new Thickness(15, 8, 15, 8),
                Margin = new Thickness(0, 0, 8, 0)
            };
            cancelBtn.Classes.Add("ModernButton");
            ...
            createBtn.Classes.Add("ModernButton");
            createBtn.Click += (s, args) =>
            {
                ...
                if (!string.IsNullOrEmpty(tagName))
                {
                    ...
                    if (mod != null) _tagService.AssignTag(mod.FileName, newTag.Id);
            ...
            // 3. ShowDialog требует владельца и является асинхронным
        ...
        private async Task LoadInstalledModsForCurrentModpackAsync()
        {
            ...
            _ = Dispatcher.UIThread.InvokeAsync(() =>
            {
                foreach (var item in InstalledModsList.Items)
                {
                    // В Avalonia ContainerFromItem доступен напрямую у ListBox/ItemsControl
                    var container = InstalledModsList.ContainerFromItem(item);
                    
                    // FrameworkElement заменяем на Control
                    UpdateModItemCheckboxVisibility(container as Control);
                }
            }, DispatcherPriority.Background);
        ...
        private async void CreateModpack_Click(object sender, RoutedEventArgs e)
        {
            ...
            var dialog = (Application.Current as App)?.Services.GetRequiredService<CreateModpackDialog>();
        ...
        private async void EditModpack_Click(object sender, RoutedEventArgs e)
        {
            ...
            var dialog = (Application.Current as App)?.Services.GetRequiredService<EditModpackDialog>();
        ...
        private async void ExportModpack_Click(object sender, RoutedEventArgs e)
        {
            ...
            // 2. Настраиваем и открываем диалог сохранения
            var file = await topLevel.StorageProvider.SaveFilePickerAsync(new FilePickerSaveOptions
            {
                Title = _localization.Get("modpack.export_title"),
                SuggestedFileName = $"{modpack.Name}.zip",
                FileTypeChoices = new[] 
                { 
            ...
            if (file != null)
            {
                try
                {
                    ...
                    await _modpackService.ExportModpackAsync(_selectedModpackId, file.Name);
        ...
        private async void ImportModpack_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (file != null)
            {
                try
                {
                    ...
                    var modpack = await _modpackService.ImportModpackAsync(file.Name);
        ...
        {
            // 1. Исправляем FindResource и ColorConverter
            // В Avalonia FindResource — это метод объекта. Color.Parse заменяет ColorConverter.
            SelectModeBtn.Background = _isSelectModeActive 
                ? (IBrush)this.FindResource("AccentBrush")! 
                : new SolidColorBrush(Color.Parse("#3d444d"));
            
            // 2. Исправляем IsVisible (bool вместо Visibility)
            SelectAllCheckBox.IsVisible = _isSelectModeActive;
            DeleteSelectedBtn.IsVisible = _isSelectModeActive && _selectedMods.Count > 0;
            
            // 3. Исправляем работу с контейнерами списка
            if (InstalledModsList.ItemsSource != null)
            {
                foreach (var item in InstalledModsList.Items)
                {
                    // В Avalonia метод ContainerFromItem доступен прямо у ItemsControl (ListBox)
                    var container = InstalledModsList.ContainerFromItem(item);
                    if (container != null)
                    {
                        // Передаем как Control (аналог FrameworkElement)
                        UpdateModItemCheckboxVisibility(container as Control);
                    }
                }
            }
        ...
        // 4. Заменяем FrameworkElement на Control
        private void UpdateModItemCheckboxVisibility(Control? container)
        {
            if (container == null) return;
            
            // FindVisualChild в Avalonia обычно реализуется через GetVisualDescendants
            var checkbox = container.GetVisualDescendants().OfType<CheckBox>().FirstOrDefault();
            
            if (checkbox != null)
            {
                checkbox.IsVisible = _isSelectModeActive;
        ...
        private void SelectAllCheckBox_Changed(object sender, RoutedEventArgs e)
        {
            ...
            if (InstalledModsList.ItemsSource != null)
            {
                foreach (var item in InstalledModsList.Items)
                {
                    ...
                    if (container != null)
                    {
                        var checkbox = FindVisualChild<CheckBox>(container as Control);
                        if (checkbox != null)
                        {
        ...
        {
            if (parent == null) return default; // Используем default вместо null

            // В Avalonia визуальное дерево обходится через GetVisualChildren()
            foreach (var child in parent.GetVisualChildren())
            {
                ...
                // Рекурсивный поиск
                var result = FindVisualChild<T>(child as Visual);
                if (result != null)
                {
                    return result;
                }
